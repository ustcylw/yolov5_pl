...
import os, sys
sys.path.insert(0, '/home/yangliwei/code/yolo/yolov5_pl')
...
from PyUtils.interface_v2 import Model
...
class YoloBody(Model):
    ...
    def forward(self, x):
        ...
        # print(f'{feat1.shape}  {feat2.shape}  {feat2.shape}')
...
if __name__ == '__main__':
    ...
    INPUT_SHAPE     = [640, 640]
    BACKBONE        = 'cspdarknet'
    PRETRAINED = False  # f'{CHECKPOINT_DIR}/last_epoch_weights.pth'
    PHI             = 's'

    model_file = r'/home/yangliwei/code/yolo/yolov5_pl/exps/000001_yolov5/lightning_logs/version_1/last.ckpt'
    model = YoloBody(
            ANCHORS_MASK,
            NUM_CLASSES,
            PHI,
    ...
    y = model(torch.FloatTensor(np.random.randint(0, 1, size=(1, 3, 640, 640))))
    
    model.load_checkpoints(model_file, mode='state_dict', device='cpu', strict=True)
    # module = torch.load(model_file, map_location=lambda storage, loc: storage)
